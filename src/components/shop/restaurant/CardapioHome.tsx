...
import { createDeliveryOrder, getDeliverySettings, getDeliveryCategories, getDeliveryZones } from "@/lib/actions/delivery";
...
interface CardapioHomeProps {
  products: any[];
  categories: any[];
  config: any;
}

export function CardapioHome({ config }: CardapioHomeProps) {
  const [settings, setSettings] = useState<any>(null);
  const [deliveryCategories, setDeliveryCategories] = useState<Category[]>([]);
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const [settingsData, categoriesData, zonesData] = await Promise.all([
          getDeliverySettings(),
          getDeliveryCategories(),
          getDeliveryZones(),
        ]);
        setSettings(settingsData);
        setDeliveryCategories(categoriesData as Category[]);
  ...
  // Theme values - prioritize StoreConfig (config) over DeliverySettings
  const primaryColor = config?.themeColor || settings?.primaryColor || "#ef4444";
  const secondaryColor = config?.secondaryColor || "#fef2f2";
  const storeName = config?.storeName || settings?.storeName || "Cardápio";
  const storeLogo = config?.logoUrl || settings?.storeLogo;
  const storeBanner = config?.bannerUrl || settings?.storeBanner;
  const headerBg = config?.headerColor || primaryColor;
  const bodyColor = config?.bodyColor || "#52525b";
  const headingColor = config?.headingColor || "#18181b";

  // Filter items by search
  const filteredCategories = deliveryCategories
    .map((cat) => ({
      ...cat,
  ...
  const cartTotal = cart.reduce((sum, item) => {
    ...
    item.customizations.forEach((c) => {
      itemTotal += (c.price || 0) * item.quantity;
    });
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: config?.backgroundColor || "#f8fafc" }}>
      {/* Header */}
      <header
        ...
        <div className="max-w-5xl mx-auto px-4 py-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="font-bold text-lg" style={{ color: headerBg === "#ffffff" || headerBg === "#fff" ? headingColor : "#ffffff" }}>{storeName}</h1>
                <div className="flex items-center gap-2 text-xs" style={{ color: headerBg === "#ffffff" || headerBg === "#fff" ? "#64748b" : "rgba(255,255,255,0.7)" }}>
                  <span className="flex items-center gap-1">
                    <Clock className="w-3 h-3" />
                    {settings?.avgPrepTime || 30}-{(settings?.avgPrepTime || 30) + 15} min
                  </span>
                  <span className="w-1 h-1 rounded-full" style={{ backgroundColor: headerBg === "#ffffff" || headerBg === "#fff" ? "#cbd5e1" : "rgba(255,255,255,0.4)" }} />
        ...
        {filteredCategories.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-sm text-slate-400 mt-2">
              Configure o cardápio em Delivery Próprio no painel admin
            </p>
          </div>
        ) : (
          <div className="space-y-8">
            {filteredCategories.map((cat) => (
              ...
              >
                <h2 className="text-xl font-bold mb-4" style={{ color: headingColor }}>{cat.name}</h2>
                {cat.description && (
                  <p className="text-sm mb-4" style={{ color: bodyColor }}>{cat.description}</p>
                )}
                  ...
                  {cart.map((cartItem) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {cartItem.notes && (
                          <p className="text-xs text-slate-500 italic">{cartItem.notes}</p>
                        )}
                        <div className="flex items-center justify-between mt-2">
                          ...
                          <span className="font-semibold">
                            R$ {((cartItem.item.price + cartItem.customizations.reduce((sum, c) => sum + (c.price || 0), 0)) * cartItem.quantity).toFixed(2)}
                          </span>
                        </div>
                      </div>
                      <button
